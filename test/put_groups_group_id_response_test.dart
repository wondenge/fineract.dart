import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for PutGroupsGroupIdResponse
void main() {
    var instance = new PutGroupsGroupIdResponse();

  group('test PutGroupsGroupIdResponse', () {
    // int officeId (default value: null)
    test('to test the property `officeId`', () async {
      // TODO
    });

    // int groupId (default value: null)
    test('to test the property `groupId`', () async {
      // TODO
    });

    // int resourceId (default value: null)
    test('to test the property `resourceId`', () async {
      // TODO
    });

    // PutGroupsGroupIdChanges changes (default value: null)
    test('to test the property `changes`', () async {
      // TODO
    });


  });

}
