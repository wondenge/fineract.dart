...
class GetSelfLoansLoanIdResponse {
  ...
  @override
  String toString() {
    return 'GetSelfLoansLoanIdResponse[id=$id, accountNo=$accountNo, status=$status, clientId=$clientId, clientName=$clientName, clientOfficeId=$clientOfficeId, loanProductId=$loanProductId, loanProductName=$loanProductName, loanProductDescription=$loanProductDescription, loanPurposeId=$loanPurposeId, loanPurposeName=$loanPurposeName, loanOfficerId=$loanOfficerId, loanOfficerName=$loanOfficerName, loanType=$loanType, currency=$currency, principal=$principal, termFrequency=$termFrequency, termPeriodFrequencyType=$termPeriodFrequencyType, numberOfRepayments=$numberOfRepayments, repaymentEvery=$repaymentEvery, repaymentFrequencyType=$repaymentFrequencyType, interestRatePerPeriod=$interestRatePerPeriod, interestRateFrequencyType=$interestRateFrequencyType, annualInterestRate=$annualInterestRate, amortizationType=$amortizationType, interestType=$interestType, interestCalculationPeriodType=$interestCalculationPeriodType, transactionProcessingStrategyId=$transactionProcessingStrategyId, timeline=$timeline, summary=$summary, ]';
  }

  GetSelfLoansLoanIdResponse.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
    id = json['id'];
  ...
  static List<GetSelfLoansLoanIdResponse> listFromJson(List<dynamic> json) {
    return json == null ? List<GetSelfLoansLoanIdResponse>() : json.map((value) => GetSelfLoansLoanIdResponse.fromJson(value)).toList();
  }

  static Map<String, GetSelfLoansLoanIdResponse> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, GetSelfLoansLoanIdResponse>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = GetSelfLoansLoanIdResponse.fromJson(value));
    }
    return map;
  }

  // maps a json object with a list of GetSelfLoansLoanIdResponse-objects as value to a dart map
  static Map<String, List<GetSelfLoansLoanIdResponse>> mapListFromJson(Map<String, dynamic> json) {
    var map = Map<String, List<GetSelfLoansLoanIdResponse>>();
     if (json != null && json.isNotEmpty) {
       json.forEach((String key, dynamic value) {
         map[key] = GetSelfLoansLoanIdResponse.listFromJson(value);
       });
     }
     return map;
  }
